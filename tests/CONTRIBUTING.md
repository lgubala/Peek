...
| Teach Peek a site with a public API | `src/sites/declared.js` — one data entry |
| Handle a site that needs real code | `src/sites/` — see the README there |
...
`scripts/build.mjs` copies `src/`, overlays `platform/<browser>/`, and
generates both manifests and Chrome's `offscreen.html` from
`build/modules.json`. Plain Node, no dependencies — the tests already need
Node, and one toolchain is one fewer thing to install and to break in CI.
