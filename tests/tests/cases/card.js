...
function show(id, reply, pageUrl, trigger) {
  ...
  /* show() called directly is always a deliberate request, so under a modifier
   * trigger it overrides the ambient guesses. Tests that are *about* those
   * guesses have to say they mean ambient mode. */
  ctx.P.settings.values.trigger = trigger || "alt";
...
module.exports = {
  ...
  async "in ambient mode, disabled sites produce nothing at all"(t) {
    const { card, api } = show("yt", RECIPE, null, "hover");
    await settle();
    t.equal(card ? card.style.display : "none", "none", "no card for a disabled destination");
    t.equal(api._sent.length, 0, "and no request");
  },

  async "in ambient mode, webmail gets a card but never a request"(t) {
    const { card, api } = show("recipe", RECIPE, "https://mail.google.com/mail/u/0/", "hover");
    await settle();
  ...
  async "asking explicitly overrides both"(t) {
    /* The point of a modifier: holding the key is the intent, so the guesses
     * Peek makes on your behalf give way to it. */
    const yt = show("yt", RECIPE, null, "alt");
    await settle();
    t.equal(yt.card.style.display, "block", "a site Peek stays off peeks when asked");

    const mail = show("recipe", RECIPE, "https://mail.google.com/mail/u/0/", "alt");
    await settle();
    t.equal(mail.api._sent.filter((m) => m.type === "peek:look").length, 1,
      "and webmail fetches when asked");
  },
